...
class StandingService
{
     ...
     * Works for both played and unplayed matches (when unplayed, excludes the match from calculation).
     */
    ...
    /**
     * Recalculate all standings for all teams in a tournament from scratch.
     */
    public function recalculateAllStandings(Tournament $tournament): void
    {
        $teams = $tournament->teams;

        // Fetch all standings for this tournament
        $standings = Standing::where('tournament_id', $tournament->id)
            ->get()
            ->keyBy('team_id');

        // Fetch all played games for this tournament
        $playedGames = Game::where('tournament_id', $tournament->id)
            ->where('is_played', true)
            ->whereNotNull('home_score')
            ->whereNotNull('away_score')
            ->get();

        // Reset and recalculate standings for all teams
        foreach ($teams as $team) {
